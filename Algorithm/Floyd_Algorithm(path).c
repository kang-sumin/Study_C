...
//최단경로 함수
void to_path(int to_q, int to_r) {
	...
	if (P[to_q][to_r] != 0) {
		to_path(to_q, P[to_q][to_r]);
		printf("v%d -> ", P[to_q][to_r]);
		to_path(P[to_q][to_r], to_r + 1);
	}
}


int main() {
	int star, end;	//시작 vertex, 끝 vertex 입력받을 변수
	...
	printf("시작 vertex와 끝 vertex를 입력하시오 >> ");
	scanf_s("%d %d", &star, &end);

	//결과
	printf("\n path(%d, %d) 결과 : ",star,end);
	path(star, end);
	printf("\n");

	//최단경로
	printf("v%d에서 v%d로 가는 최단경로 : ",star, end);
	printf("v%d -> ", star);
	to_path(star, end);
	printf("v%d", end);
